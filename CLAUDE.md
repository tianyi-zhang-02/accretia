...
**Monthly tracking (the home screen, deliberately light).** The **Track** section opens on a one-month **check-in** (`components/ledger/month-check-in.tsx`): three labeled money fields, placeholders from the last three months' average, a "same as last month" chip for income, a live answer as you type ("you kept $X · your plan expects $Y a month"), an explicit Save, and a 12-dot year strip to jump between months. It opens on *last* month until that's logged (its numbers are final), then on this month (`checkInTarget`). Beside it, **Am I on track?** (`track-status.tsx`) holds the latest real net worth against the plan's interpolated value for that month (`planAt`), a plain-SVG actual-vs-plan sparkline, year-to-date kept/spent vs plan, and the projection reduced to one sentence with an "Open plan" link. The 12×3 grid and the CSV tools sit behind an "All months & spreadsheet" disclosure — nobody should face 36 empty cells to log one month. It records three numbers a month — take-home income, spending, month-end net worth — typed in or imported from a CSV template, and produces a year report (totals, savings rate, plan vs actual, print-to-PDF) plus a one-click "use these numbers in my plan" calibration. Logic is pure and tested in `src/lib/ledger/ledger.ts`. **Coarse or fine (owner's call, 2026-09).** "Money out" is either one total or **By category** — a FIXED list of ten (`CATEGORY_IDS`: housing, utilities, food, transport, insurance, health, family, fun, debt, other). Each category can be opened into **line items** (`SUBCATEGORIES`, ids like `housing.propertyTax`, ~40 in all — mortgage payment / rent / property tax / HOA / upkeep; health / auto / home / life insurance; …), also a fixed list. One invariant, enforced in `normalize()` (schema transform + `upsert`): **totals are derived from the finest level filled in** — a category with line items IS their sum (its input goes read-only), and with categories `spending` IS their sum. A new month opens in whichever mode the previous one used; "Fill blanks from last month" copies the fixed bills; the year report gains a "Where it went" breakdown; the "Where it went" report nests line items under each category (share of the category, plus what was left un-itemized). CSV gains optional category columns (English ids + Chinese aliases; `spending` may be omitted) and line-item columns matched by id, case-insensitively, with an optional human label after the id — `housing.hoa (物业费)` — so the full template is readable in either language. For plan comparisons the engine now reports `housingCosts` and `mortgageBalance` per year (outputs only — the math is unchanged): the engine pays for the home outside `expenses`, but a person logs the mortgage as spending, so `planAt` / `yearReport.plan` add it back. This is still NOT the old tracker coming back: no accounts, no transactions, no custom categories or items, no per-purchase entries.
...
- **End-to-end encrypted.** `lib/cloud/crypto.ts` (WebCrypto: PBKDF2-SHA256 600k → AES-256-GCM, fresh IV per write). **Envelope v2 (2026-09, owner's call "忘记密码可以通过邮箱找回" → answered with recovery codes, not server-side reset):** the data is encrypted with a random DATA key, which is stored wrapped under the passphrase-derived key AND under a key derived from a random 32-character base32 **recovery code** shown once when the passphrase is set. Either secret opens the vault; the server holds neither. Recovering with the code retires the old passphrase (a new one must be chosen); "Get a new code" retires the old code; both take effect on the next upload. v1 envelopes (no code) still open and upgrade to v2 when a code is added. The derived keys are non-extractable; the data key is extractable (it must be re-wrapped on a passphrase/code change) and nothing ever calls `exportKey` — pinned by the isolation test. Passphrase, code and keys live in memory only — never storage, never the network. **There is no email-based reset and must never be one**: that would require the server to hold a key. With both secrets lost the cloud copy is gone; the UI offers "delete it and start over" from this device's data.
...
- **Sign-in is a guided three-step flow** (email → open the link → passphrase) in `cloud-sync.tsx`: a "check your inbox" state with a 60s resend cooldown (Supabase's own throttle), a code box only behind "my email shows a code", 429s and dead links explained in words. The shell (`simulator-client.tsx`) reads the URL once on load: arriving from the email jumps straight to the account panel (`arrival` = `'ok' | 'failed'`), and a dead link's parameters are tidied out of the address bar. The header shows **Sign in** / the signed-in email. **The Data section stays mounted (hidden) once opened** so the unlocked key and the auth listener survive tab switches — but it is never mounted before that, so a visitor who doesn't open it still never loads the network code. The panel also says whether this device **matches the cloud copy or has changes to upload** (an in-memory signature; sync itself stays manual).
