...
一切都在浏览器里运行。**没有后端、没有数据库、没有账号，任何数据都绝不上传。** 方案——以及你的每月记账(如果你记的话)——默认保存在你自己的设备上(此浏览器的本地存储)，下次打开还在——不想留就取消勾选**「在本设备保存」**。要长期用的话，**「备份全部数据」**会把所有方案和每个月的记账打包成一个文件，换任何设备都能用**「从备份恢复」**找回来。
