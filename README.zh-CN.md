...
_算出你哪一年可以不上班，并且每个月看一眼自己有没有跟上。_ 一个单页、**纯前端**的应用，分三个栏目：**记账**（首页：每月花一分钟填三个数——进账、花销、净资产——填完当场告诉你「这个月攒下多少、规划要求多少」；「我跟上规划了吗？」卡片把真实净资产和规划放在一起比；年底出一份可打印的年度报表）、**规划**（财富推算模拟器）和**数据**（备份、恢复、可选的加密同步）。推算是尺子，记账才是习惯。
